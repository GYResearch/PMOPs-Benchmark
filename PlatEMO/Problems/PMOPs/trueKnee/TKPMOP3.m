...
y=[0:0.001:1];
[u,v]=meshgrid(x,y);
mx1 = 1 + exp(sin(A.*power(u, B).*pi+pi./2))./(power(2, S).* A);
mx2 = 1 + exp(sin(A.*power(v, B).*pi+pi./2))./(power(2, S).* A);
ma=(mx1.*mx2)./2;
m = 2.^ma;
%m=m1
f1=2.*m.*(1-cos(u.*pi./2)).*(1-cos(v.*pi./2));
f2=2.*m.*(1-cos(u.*pi./2)).*(1-sin(v.*pi./2));
f3=2.*m.*(1-sin(u.*pi./2));
plot3(f1,f2,f3);
hold on
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mknee = zeros(nrf,1);
for i=1:M-1
    mk(:,i) = 1 + exp(sin(A.*power(vark(:,i), B).*pi+pi./2))./(power(2, S).* A);
end 
Tr = mk'; %% transposition
if M>2
...
mknee =2.^(mknee'); %% transposition
%% construct the shape functions: 
%% min (g3) = 1... 
PopObj = ones(nrf,M);    
for i=1:M
    PopObj(:,i) = PopObj(:,i).*(1.0+1.0).* mknee;  %% (1.0+1.0)
 end
 for i=1:M
               for j=1:M-i
