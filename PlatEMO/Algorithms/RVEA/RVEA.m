...
%--------------------------------------------------------------------------
    ...
    while Global.NotTermination(Population)
        ...
        %%% adding dominance is better, but the original did not.
        %          [Ns,~]   = size(Population.decs);
        %          [FrontNo,MaxFNo] = NDSort(Population.objs,Ns);
        %          Population = Population(FrontNo ==1);
        %          if Global.evaluated == Global.evaluation
        %             ss = Population.objs;
        %             save('C:\Users\Lursonkj\Desktop\Data-driven 4.1\PlatEMO-master\PlatEMO-master\PlatEMO\ALL_Data\Popss.mat','ss');
        %         end
    end
end